...
if uploaded_file:
    ...
    if "Year" in data.columns and "Subscribers" in data.columns:
        ...
        ensemble_model = VotingRegressor(estimators=[("lr", linear_model), ("rf", random_forest)]).fit(X, y)

        # Model selection
        model_choice = st.selectbox("Select Prediction Model", ["Linear Regression", "Random Forest", "Ensemble Model"])
...
if st.checkbox("Generate Sample Data"):
    sample_data = pd.DataFrame({
        "Year": range(2010, 2021),
        "Subscribers": [5000, 8000, 12000, 17000, 23000, 31000, 40000, 51000, 65000, 82000, 102000]
    })
